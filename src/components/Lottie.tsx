...
export const Lottie: VoidComponent<LottieProps> = (props) => {
  ...
  createEffect(() => {
    ...
    const listeners: LottieListener[] = [
      ...
      (listener): listener is LottieListener => listener.handler !== undefined
    );

    if (!listeners.length) return;

    listeners.forEach((listener) =>
      a.addEventListener(listener.name, listener.handler)
    );
  });
  ...
  const setupAnimation = () => {
    setAnimation((s) => {
      ...
      if (!local.path && !local.animationData)
        throw new Error(
          "You must provide animationData or path to the animation data"
        );
  ...
  const getDuration = (inFrames?: boolean) =>
    animation()?.getDuration(inFrames) ?? 0;
